...
<section class="cosiness">
    <div class="cosiness__content">

        <h2 class="section__title">Впусти уют в свой дом</h2>

        <div class="cosiness__body">

            <div class="cosiness__left">
            	<?
            	$count = 0;
            	foreach($arResult['ITEMS'] as $i => $arItem):
            	$count++;?>
                <div class="cosiness__text-wrap">
                    <span class="cosiness__text-title"><?=$arItem['~NAME'];?></span>
                    <p class="cosiness__text"><?=$arItem['FIELDS']['PREVIEW_TEXT']?></p>
                </div>
                <a href="<?php echo $arItem['PROPERTIES']['LINK']['VALUE']; ?>" class="cosiness__btn">Подробнее</a>
                <?
            if($count == 1) {
            	break;
            }
            ...
            <div class="cosiness__right">
            	<?
            	$count = 0;
            	foreach($arResult['ITEMS'] as $i => $arItem):
            	$count++;
            	if($count > 1) {?>
                <div class="cosiness__item">
                    <p class="cosiness__text"><?=$arItem['~NAME'];?></p>
                    <div class="cosiness__icon-wrap">
                        <img src="<?php echo $arItem['FIELDS']['PREVIEW_PICTURE']['SRC']; ?>" alt="#" class="cosiness__icon">
                    </div>
                </div>
               	<? } endforeach;?>
            </div>
